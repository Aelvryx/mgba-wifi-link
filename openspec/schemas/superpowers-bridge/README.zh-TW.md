...
flowchart TD
    ...
    subgraph Apply ["⚙️ APPLY — 7 個順序步驟(requires: plan,tracks: tasks.md)"]
        ...
        A6["<b>6. Completion</b><br/><i>finishing-a-development-branch</i><br/>🏁 PR ready/merge 是最後一步"]
...
APPLY ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  3. openspec-verify-change → verify.md ◄┐
                              │           │ blocking → 回去修
                              ▼           │
  4. retrospective.md(PR ready/merge 之前;hot context)
  5. openspec archive -y(sync delta + 搬 folder)
  6. superpowers:finishing-a-development-branch(🏁 PR ready/merge 是最後一步)
```

> **時序註記**(完整理由見下方「設計觸點 #6」):
> - `verify.md` 在 graph 上宣告 `requires: plan`,但實際產在 apply step 3 內。
> - approved plan 需要 GitHub Actions、protected-check evidence 或 external review 時,可提早開 working draft PR;實作期間維持 draft。
> - `retrospective.md` 宣告 `requires: verify`,並依 Step 4 在 PR ready/merge **之前**產出。Archive commit push 到同一個 draft PR,讓最終 diff 包含完整 archived cycle(所有 artifact 完成、spec 已 sync、change folder 在 `archive/`)。
...
> **Steps 4–6 是 canonical completion 順序:retro → archive → PR ready/merge。Working draft PR 可以先存在作為 CI/review surface,但 incomplete cycle 不可 ready 或 merge。**
...
在 PR ready/merge **之前**寫好,讓 retro 跟其他 artifact 一起落在同一個 PR diff。Working draft 可以已經存在。

#### 5. Archive — `openspec archive -y`(或 `/opsx:archive`)

把 delta spec sync 到 `openspec/specs/<capability>/spec.md`、把 change 目錄搬到 `openspec/changes/archive/YYYY-MM-DD-<name>/`。在 PR ready/merge **之前**跑完;若已提早開 draft,把 archive commit push 到同一個 PR。最終 diff 反映完整的 archived cycle 狀態(所有 artifact 完成、spec 已 sync、folder 在 `archive/`)。

#### 6. Completion — `superpowers:finishing-a-development-branch`

確認 tests 全綠、呈現 merge / PR / keep-branch / discard 選項,並在適當時清理 worktree。**PR ready 與 merge 是最後一步** —— early draft 合法,但 retro、archive 與 final protected checks 完成前不可 ready 或 merge。若 draft 已存在,持續更新同一個 PR,不要建立重複 PR。
